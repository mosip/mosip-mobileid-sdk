  ...
  try {
    const granted = await PermissionsAndroid.request(
      'android.permission.READ_EXTERNAL_STORAGE',
      {
        ...
        message: 'Needs permission of storage to validate photos',
...
export default function App() {
  ...
  RNFS.readFile(
    '/storage/emulated/0/Download/Janardhan_004.jpg',
    'base64'
  ).then((data) => {
    capturedImage = data;
  ...
  RNFS.readFile(
    '/storage/emulated/0/Download/Janardhan_005.jpg',
    'base64'
  ).then((data) => {
    vcImage = data;
  ...
  const performAuth = async () => {
    ...
    await faceAuth(capturedImage, vcImage).then(setResult).catch(setResult);
  ...
  return (
    <View style={styles.container}>
      <Button
        title="Request storage permissions"
        onPress={requestStoragePermission}
      />
      <Button title="Perform Auth" onPress={performAuth} />
      {result === true ? (
        <Text style={{ color: 'green', fontSize: 40 }}>Result is true</Text>
      ) : (
        <Text style={{ color: 'red', fontSize: 40 }}>Result is false</Text>
      )}
    </View>
  );
}
